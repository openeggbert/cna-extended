...
its Phase 12 (remaining "extend later if wanted" items) DONE (A/B/C/D/E all complete,
2026-07-15). There is no remaining phase in either plan.** An independent audit (Codex,
`audit.md`, 2026-07-14) reviewed the completed port and `World3DEXT`; `plan3d.md`'s Phase 10
tracks those findings (see that file for current per-finding status). Phases 11 and 12 were
both separate, user-requested follow-up work (not audit findings) — see that file's own
entries for the full scope/sequencing/rationale record. See section 4 for how `plan.md`'s
two former blockers (the `TilemapRenderer`/`TilemapWorldRenderer`
...
8. `Tilemaps3DEXT` — a sparse `Tilemap3DEXT` voxel grid (tile ID 0 = empty),
   ...
   (hand-built array data, plus a JSON file reader since Phase 12 C, 2026-07-15), a
   standalone `TilemapRenderer3DEXT` reusing a `CubeMeshRenderSystemEXT::DrawCubeEXT`
   extraction per visible tile, plus (since Phase 12 E, 2026-07-15) an opt-in
   `TilemapChunkRenderer3DEXT` batching visible tile faces into one draw call per
   `(chunk, texture)` pair instead of one per tile, and (since Phase 12 B, 2026-07-15)
   `TilemapCollisionActor3DEXT`/`RegisterTilemapCollisionActorsEXT` bridging populated tiles
   into `CollisionWorld3DEXT` in bulk.
...
Full test suite: **2363/2365 passing** (2 pre-existing skips predating `plan3d.md`,
...
- ~~`TilemapRenderer3DEXT`: per-chunk geometry batching.~~ Done (Phase 12 E) — new
  `TilemapChunkRenderer3DEXT`, an opt-in alternative (not a replacement) grouping visible
  tile faces into one draw call per `(chunk, texture)` pair.

**Phase 12 is now fully complete — all 5 "extend later if wanted" items from Phase 11 are
done.** There are no more known scope gaps in either `plan.md` or `plan3d.md`; any further
work would be new user-requested scope, not a documented backlog item.
