...
//
// No upstream tests exist for ExpandTransition. Fresh tests below. A plain
...
namespace CNA::Extended::Screens::Transitions
{
    ...
    namespace
    {
        class ExpandTransitionTest : public ::testing::Test
        {
        protected:
    ...
    TEST_F(ExpandTransitionTest, Draw_AtStart_DoesNotThrow)
    {
        ExpandTransition transition(graphicsDevice, Color::Black);
        GameTime gameTime;
        EXPECT_NO_THROW(transition.Draw(gameTime));
    }

    TEST_F(ExpandTransitionTest, Draw_MidTransition_DoesNotThrow)
    {
        ExpandTransition transition(graphicsDevice, Color::Black);
    ...
    TEST_F(ExpandTransitionTest, Draw_AfterFullOutHalf_DoesNotThrow)
    {
        ExpandTransition transition(graphicsDevice, Color::Red, 1.0f);
    ...
    TEST_F(ExpandTransitionTest, GetColorProperty_ReturnsConstructedColor)
    {
        const ExpandTransition transition(graphicsDevice, Color::Green);
        EXPECT_EQ(transition.getColorProperty(), Color::Green);
    }
}
