...
{
    KeyboardEventArgs::KeyboardEventArgs(Keys key, const KeyboardState& keyboardState) : Key_(key), Modifiers_(KeyboardModifiers::None)
    {
        if (keyboardState.IsKeyDown(Keys::LeftControl) || keyboardState.IsKeyDown(Keys::RightControl))
        {
            Modifiers_ |= KeyboardModifiers::Control;
        }

        if (keyboardState.IsKeyDown(Keys::LeftShift) || keyboardState.IsKeyDown(Keys::RightShift))
        {
            Modifiers_ |= KeyboardModifiers::Shift;
        }

        if (keyboardState.IsKeyDown(Keys::LeftAlt) || keyboardState.IsKeyDown(Keys::RightAlt))
        {
            Modifiers_ |= KeyboardModifiers::Alt;
        }
    }

    std::optional<char> KeyboardEventArgs::getCharacterProperty() const
    {
        return ToChar(Key_, Modifiers_);
    }

    std::optional<char> KeyboardEventArgs::ToChar(Keys key, KeyboardModifiers modifiers)
    {
        const bool isShiftDown = (modifiers & KeyboardModifiers::Shift) == KeyboardModifiers::Shift;
        ...
        if (key == Keys::Enter) return static_cast<char>(13);
        if (key == Keys::Back) return static_cast<char>(8);
